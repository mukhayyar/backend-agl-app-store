...
@app.get("/push-to-agl.sh", response_class=PlainTextResponse, include_in_schema=False)
def serve_push_script():
    """Serve the developer push script for download."""
    return _PUSH_SCRIPT_CONTENT
...
async def upload_bundle(
    ...
    if not file.filename or not file.filename.endswith(".flatpak"):
        raise HTTPException(status_code=400, detail="File must be a .flatpak bundle (use `flatpak build-bundle` to create one)")
    ...
    with tempfile.NamedTemporaryFile(suffix=".flatpak", delete=False) as tmp:
        tmp.write(data)
        tmp_path = tmp.name

    REPO = "/srv/flatpak-repo"
    try:
        # Peek at the bundle to get app_id before importing
        info = subprocess.run(
            ["flatpak", "info", "--show-ref", tmp_path],
            capture_output=True, text=True
        )
        if info.returncode != 0:
            # fallback: use flatpak build-show-info
            info = subprocess.run(
                ["ostree", "show", "--repo=" + tmp_path + ".ostree", "refs"],
                capture_output=True, text=True
            )

        # Import into staging area of main repo (not published yet)
        ...
        if result.returncode != 0:
            raise HTTPException(
                status_code=422,
                detail=f"Bundle import failed: {result.stderr[:400]}"
            )

        # Extract the app_id from the imported ref (format: app/com.x.Y/arch/branch)
        imported_ref = result.stdout.strip()
        # flatpak build-import-bundle outputs a line like: Imported: app/com.x.Y/x86_64/stable
        match = _re.search(r'app/([^/]+)/[^/]+/[^\s]+', result.stdout + result.stderr)
        if not match:
            # Try to get it from the filename
            match = _re.match(r'([a-zA-Z][a-zA-Z0-9_-]*(?:\.[a-zA-Z][a-zA-Z0-9_-]*){2,})\.flatpak$',
                              file.filename)
            app_id = match.group(1) if match else file.filename.replace(".flatpak", "")
        else:
...
def issue_flat_manager_token(req: IssueTokenRequest, _: bool = Depends(require_admin_key)):
    ...
    build_script = (
        ...
        'BUILD_URL=$(flat-manager-client --token "$FLAT_MANAGER_TOKEN" create "$FLAT_MANAGER_URL" stable "$APP_ID")\n'
        'BUILD_ID=$(echo "$BUILD_URL" | grep -oP "build/\\K[0-9]+")\n'
        'echo "  Build URL : $BUILD_URL"\n'
        'echo "  Build ID  : $BUILD_ID"\n\n'
        'echo "[4/6] Uploading build to AGL hub..."\n'
        'flat-manager-client --token "$FLAT_MANAGER_TOKEN" push "$BUILD_URL" "./$REPO_DIR"\n\n'
        'echo "[5/6] Committing build (waits for OSTree ingest)..."\n'
        'flat-manager-client --token "$FLAT_MANAGER_TOKEN" commit --wait "$BUILD_URL"\n\n'
